...
DOMAINS = [
    "scannerapp.dev", "docsapp.dev",  # promptsite.app + [retired brand] domains removed (PII/anon)
    "redacted.com", "redacted.com", "Operator.com", "hailports.com", "persona1.com",
]
DOMAIN_TIERS = [45, 30, 14, 7, 3, 1]

# CDP profiles whose login keeps a revenue/posting lane alive; stale cookie file = lane dark.
SESSIONS = [
    ("gumroad", "~/.chrome-cdp-profile-gumroad/Default/Cookies", 21, ""),
    ("x-persona1", "~/.chrome-cdp-profile/Default/Cookies", 14,
     str(ROOT / ".venv/bin/python") + " -m agents.harvest_x_session"),
]
EARNING_SIGNALS = [   # any one of these being touched recently = the money loop is alive
    "data/hustle/broken_site_outreach_queue.jsonl",
    "products/self_serve/reports/scans.jsonl",
    "products/self_serve/hailports_scans.jsonl",  # LIVE funnel activity (scans.jsonl above is dead)
    "data/hustle/SHADOW_STRIKE_BOARD.md",
]
