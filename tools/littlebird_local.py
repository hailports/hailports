...
# ── Headless WEB bridge (app.littlebird.ai IDB via CDP) — preferred over the desktop scrape ──
_LB_WEB_JSON = os.path.expanduser("~/.openclaw/workspace/CompanyA-local/digests/LITTLEBIRD_NOTES.json")


def _lb_web_data():
    """Load the web-bridge digest; refresh it live if missing/stale (>1h). None if unavailable."""
    import json as _json, time as _time, subprocess as _sp
    try:
        fresh = os.path.exists(_LB_WEB_JSON) and (_time.time() - os.path.getmtime(_LB_WEB_JSON) < 3600)
        if not fresh:
            base = os.path.expanduser("~/claude-stack")
            r = _sp.run(["node", os.path.join(base, "tools/littlebird_web_bridge.js"), "--json"],
                        cwd=base, capture_output=True, text=True, timeout=90)
            out = (r.stdout or "").strip()
            if out.startswith("{"):
                return _json.loads(out)
        return _json.loads(open(_LB_WEB_JSON).read())
    except Exception:
        return None


def _lb_web_handle(tool_name, tool_input):
    """Serve the main Littlebird read tools from the web IDB (app-free). None = fall through."""
    if tool_name not in ("littlebird_recent_notes", "littlebird_search", "littlebird_transcripts",
                         "littlebird_daily_summary", "littlebird_my_meetings", "littlebird_meeting_events"):
        return None
    d = _lb_web_data()
    if not d or not (d.get("meetings") or d.get("journals")):
        return None  # web bridge down → fall through to desktop scrape
    meetings, journals = d.get("meetings", []), d.get("journals", [])
    fmt = lambda s: str(s or "")[:16].replace("T", " ")
    if tool_name == "littlebird_daily_summary":
        if not journals:
            return None
        return "\n\n".join(f"## {fmt(j.get('date'))}\n{j.get('text','')}" for j in journals[:5])[:4000]
    if tool_name == "littlebird_transcripts":
        q = (tool_input.get("query") or "").lower()
        ms = [m for m in meetings if not q or q in (m.get("name", "") + m.get("transcript", "")).lower()]
        out = [f"### {m['name']} — {fmt(m.get('sortAt'))}\n{(m.get('transcript') or '(no transcript)')[:3000]}" for m in ms[:int(tool_input.get('count', 3))]]
        return ("\n\n".join(out) or "No transcripts found.")[:6000]
    if tool_name == "littlebird_search":
        q = (tool_input.get("query") or "").lower()
        hits = [m for m in meetings if q in (m.get("name", "") + " " + m.get("summary", "") + " " + m.get("tldr", "")).lower()]
        out = [f"- {fmt(m.get('sortAt'))} | {m['name']} — {m.get('tldr', '')[:160]}" for m in hits[:15]]
        return ("Littlebird matches:\n" + "\n".join(out)) if out else f"No Littlebird notes matching '{tool_input.get('query','')}'."
    # recent_notes / my_meetings / meeting_events — recent meeting list with tldr
    n = int(tool_input.get("count", 10))
    out = []
    for m in meetings[:n]:
        out.append(f"### {m['name']} — {fmt(m.get('sortAt'))}{' · ' + m['sourceApp'] if m.get('sourceApp') else ''}")
        if m.get("attendees"):
            out.append(f"- attendees: {', '.join(str(a) for a in m['attendees'][:8])}")
        if m.get("tldr"):
            out.append(f"- {m['tldr']}")
        out.append("")
    return "\n".join(out)[:4000] or "No recent Littlebird notes."
redacted_WORKSPACE = os.path.expanduser("~/.openclaw/workspace/CompanyA-local")
SYNCED_ROOT = Path(os.path.expanduser("~/Library/Application Support/Littlebird/synced_profiles"))
NOTE_PATTERN = (
...
def _query_implies_live_window(query: str) -> bool:
    text = str(query or "").lower()
    return any(term in text for term in ("today", "tomorrow", "this week", "next week", "latest", "recent", "current"))
...
_LB_STRINGS_CACHE = {"ts": 0.0, "raw": None}
_LB_STRINGS_LOCK = threading.Lock()
_LB_STRINGS_TTL = 60.0


def _all_littlebird_strings():
    """Read all local Littlebird strings once for structured week extraction.

    Memoized ~60s. A briefing dispatches ~9-13 of these concurrently (asyncio.gather
    over run_in_executor real OS threads), each a full find|strings disk scan. Double-
    checked locking collapses the simultaneous burst into ONE scan; byte-identical output."""
    now = _time.time()
    c = _LB_STRINGS_CACHE
    if c["raw"] is not None and (now - c["ts"]) < _LB_STRINGS_TTL:
        return _clean_lines(c["raw"], 100000)
    with _LB_STRINGS_LOCK:
        now = _time.time()
        if c["raw"] is not None and (now - c["ts"]) < _LB_STRINGS_TTL:
            return _clean_lines(c["raw"], 100000)
        cmd = (
...
def _clean_littlebird_line(line):
    ...
    if re.fullmatch(r"[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}", text, re.I):
        return ""
    if re.fullmatch(r"(sources\d*|modalitiesUsedA?|todoResultsA?|toolCallsA?|toolCall|agent|available_tools)", text, re.I):
        return ""
...
def _days_from_input(tool_input, default=7):
    raw = " ".join(str(tool_input.get(k, "")) for k in ("days", "date_range", "query"))
    match = re.search(r"last\s+(\d{1,2})\s+days?", raw, re.I)
    if match:
        return max(1, min(int(match.group(1)), 60))
    try:
        return max(1, min(int(tool_input.get("days", default)), 60))
    except Exception:
        return default
...
def _query_terms(query):
    return [
        term.lower()
        for term in re.findall(r"[A-Za-z0-9][A-Za-z0-9_.-]{2,}", str(query or ""))
        if term.lower() not in {"the", "and", "for", "with", "from", "this", "that", "last", "days", "week", "next"}
    ][:8]
...
def _record_text(record):
    return " ".join([
        ...
        " ".join(record.get("actionish_lines") or []),
        " ".join(record.get("transcript_excerpt") or []),
    ])


def _clean_records_for_window(limit=40, query="", days=7, start_date="", end_date=""):
    dates = _date_terms(start_date=start_date, end_date=end_date, days=days)
    blocks = _extract_week_blocks(_all_littlebird_strings(), dates, max_blocks=500)
    records = _clean_week_records(blocks, limit=max(limit * 2, 80))
    terms = _query_terms(query)
    if terms:
        filtered = [
            rec for rec in records
            if any(term in _record_text(rec).lower() for term in terms)
        ]
        if filtered:
            records = filtered
...
def _render_recent_clean_records(records, max_records=10):
    if not records:
        return "No cleaned Littlebird notes found for that window."
    lines = ["Cleaned Littlebird notes:"]
    for rec in records[:max_records]:
        lines.append(f"- {rec.get('date') or 'undated'} | {rec.get('title')}")
        if rec.get("summary"):
            lines.append(f"  Summary: {rec['summary'][:320]}")
        for action in (rec.get("actionish_lines") or [])[:3]:
            lines.append(f"  Signal: {action[:220]}")
    return "\n".join(lines)


def _render_clean_action_items(records, max_items=20):
    hits = []
    seen = set()
    for rec in records:
        lines = rec.get("actionish_lines") or []
        if not lines and rec.get("summary") and re.search(r"follow|next|need|pending|waiting|review|block|risk", rec["summary"], re.I):
            lines = [rec["summary"]]
        for raw in lines:
            line = _clean_littlebird_line(raw)
            if not line:
                continue
            key = line[:220].lower()
            if key in seen:
                continue
            seen.add(key)
            hits.append(f"- {rec.get('date') or 'undated'} | {rec.get('title')}: {line[:300]}")
            if len(hits) >= max_items:
                return "Cleaned Littlebird action/decision signals:\n" + "\n".join(hits)
    if not hits and records:
        return _render_recent_clean_records(records, max_records=8)
    return "No cleaned Littlebird action/decision signals found for that window."


def _clean_search_results(query, max_results=30):
    raw = _search_all_data(INDEXEDDB_PATH, query, max_results=max_results)
    hits = []
    seen = set()
    if raw and raw != "No results found.":
        for line in raw.splitlines():
            ...
            if not cleaned:
                continue
            key = cleaned[:220].lower()
            if key in seen:
                continue
            seen.add(key)
            hits.append(f"- {cleaned[:360]}")
            if len(hits) >= max_results:
                break
    if hits:
        return "Cleaned Littlebird search hits:\n" + "\n".join(hits)

    window_days = 3 if _query_implies_live_window(query) else 14
    records = _clean_records_for_window(limit=12, query=query, days=window_days)
    if records:
        return _render_recent_clean_records(records, max_records=8)
    return "No cleaned Littlebird results found."
...
def _extract_title(lines):
    for line in lines:
        if "name" in line.lower():
            ...
            if cleaned and len(cleaned) <= 120 and not re.search(r"^[\W_]+$", cleaned):
                return cleaned
    for line in lines:
        cleaned = _clean_littlebird_line(line)
        if cleaned and re.search(r"Salesforce|Dashboard|Data|Sprint|Warranty|Sandbox|Deployment|Review|Discussion|Standup|Migration|Meeting|CompanyA|Littlebird|Zoom|Transcript|Summary|Action", cleaned, re.I):
            return cleaned[:120]
    return ""
...
def _clean_week_records(blocks, limit=120):
    ...
    for block in blocks:
        ...
        if not title and not tldr and not transcript and not actions:
            continue
        if not title and len(clean_lines) < 4:
            continue
...
def _reconcile_meetings_with_calendar(user_id="", days=7):
    ...
    try:
        ...
        days_back = 7
    # Default to current-week scope so summary runs do not pull in stale meetings
    # from prior weeks unless the caller explicitly broadens the window.
    if days_back == 7:
        days_back = 0
        ...
        try:
            script = '''
                tell application "Calendar"
                    ...
                    set weekStart to today
                    repeat while (weekday of weekStart) is not Monday
                        set weekStart to weekStart - 1 * days
                    end repeat
                    set output to ""
                    repeat with c in calendars
                        set evts to (every event of c whose start date >= weekStart)
        ...
        try:
            script = '''
                tell application "Microsoft Outlook"
                    ...
                    set calEvents to (every calendar event whose start time >= weekStart and start time < (today + 1 * days))
                    set output to ""
...
class LittlebirdLocalTool(BaseTool):
    ...
    async def handle(self, tool_name, tool_input):
        ...
        # HEADLESS (2026-06-12): prefer the Littlebird WEB bridge digest (app.littlebird.ai
        # IDB via CDP — no desktop app). Falls through to the legacy desktop LevelDB scrape
        # if the web data is absent/stale, so nothing breaks.
        try:
            _wr = await loop.run_in_executor(None, _lb_web_handle, tool_name, tool_input)
        except Exception:
            _wr = None
        if _wr is not None:
            return _wr
        ...
        if tool_name == "littlebird_recent_notes":
            ...
            days = _days_from_input(tool_input, default=7)
            records = await loop.run_in_executor(None, _clean_records_for_window, max(int(count), 10), query, days, "", "")
            return _render_recent_clean_records(records, max_records=int(count))[:4000]
            ...
            result = await loop.run_in_executor(None, _clean_search_results, query)
            return result[:4000]

        elif tool_name == "littlebird_transcripts":
            count = tool_input.get("count", 5)
            result = await loop.run_in_executor(None, _extract_strings, INDEXEDDB_PATH, "transcript|personifiedTranscript", count * 3)
            if not result:
                records = await loop.run_in_executor(None, _clean_records_for_window, 20, "transcript personifiedTranscript", 7, "", "")
                return _render_recent_clean_records(records, max_records=min(int(count), 6))[:4000] if records else "No transcripts found."
            return result[:4000]

        elif tool_name == "littlebird_action_items":
            query = tool_input.get("query", "")
            decoded = await loop.run_in_executor(None, _decode_chat_notes, query, 60, 60)
            ...
            records = await loop.run_in_executor(None, _clean_records_for_window, 20, "daily summary work summary recap today latest", 3, "", "")
            summaries = [
                rec for rec in records
                if re.search(r"daily|work summary|recap|summary", _record_text(rec), re.I)
            ] or records
            return _render_recent_clean_records(summaries, max_records=6)[:4000]
            ...
            selected = str((tool_input or {}).get("user") or "Operator").strip().lower() or "Operator"
            ...
            profiles = {}
            for profile_dir in sorted(SYNCED_ROOT.glob("*")) if SYNCED_ROOT.exists() else []:
                if not profile_dir.is_dir():
                    continue
                profiles[profile_dir.name] = {
                    "indexeddb": (profile_dir / "IndexedDB" / "file__0.indexeddb.leveldb").exists(),
                    "event_db": (profile_dir / "event_tracking.db").exists(),
                    "meetings": (profile_dir / "meetings-v1.json").exists(),
                }
            return json.dumps({
                "running": running,
                "event_db": db_exists,
